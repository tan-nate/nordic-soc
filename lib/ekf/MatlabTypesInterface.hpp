...
public:
    ...
    MATLABHandleObject(std::shared_ptr<ControllerType> matlabPtr, std::u16string clsName) : MATLABObject<ControllerType>(matlabPtr, clsName) {
    }

    MATLABHandleObject(std::shared_ptr<ControllerType> matlabPtr, matlab::data::Array obj) : MATLABObject<ControllerType>(matlabPtr, obj) {
    }

    bool isvalid() {
        std::vector<matlab::data::Array> _args = { this->m_object };
        matlab::data::TypedArray<bool> _result_mda = this->m_matlabPtr->feval(u"isvalid", _args);
